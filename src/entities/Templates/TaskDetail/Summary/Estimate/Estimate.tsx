...
export const Estimate: FC<ITaskSummaryProps> = ({ taskState, refetchTaskData, dataProject, isUserTeamTemplate }) => {
	const {
		...
		cancelation,
		onSave,
	} = useEstimateController({ taskState, refetchTaskData, dataProject, isUserTeamTemplate });
				...
				<View padding={4} direction='row' gap={2} width='320px'>
					...
					<View paddingTop={4} direction='row' justify='end' align='end' width='100%' gap={1}>
						...
						<Button
							...
							color='primary'
							onClick={onSave}
							disabled={inputValue?.length !== 5 || typeWork === undefined}
						>
							Сохранить
						</Button>
					</View>
