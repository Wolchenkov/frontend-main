...
export const calculateLowDyControlPointShift = (dx: number, dy: number, maxShift = MAX_Y_CONTROL_POINT_SHIFT) => {
	if (dx > 0) return 0;
	const sign = dy < 0 ? -1 : 1;
	const value = Math.round(maxShift * Math.pow(0.9, Math.pow(1.2, Math.abs(dy) / 10)));
...
export const calculateCanvasDimensions = ({
	...
	const canvasHeight = absDy + 2 * boundingBoxBuffer.vertical + 10;

	return { canvasWidth, canvasHeight };
};

export const calculateFixedLineInflectionConstant = (absDx: number, absDy: number) => {
	const WEIGHT_X = 4;
	const WEIGHT_Y = 0.8;
...
export const calculateControlPointsWithoutBoundingBox = ({
	...
	const fixedLineInflectionConstant = calculateFixedLineInflectionConstant(absDx, absDy);
	const lowDyYShift = calculateLowDyControlPointShift(dx, dy);
	const lowDyXShift = Math.abs(calculateLowDyControlPointShift(dx, dy, MAX_X_CONTROL_POINT_SHIFT));
